...
class DriveAPI {
  constructor() {
     ...
     */
    this.restEndpoint = "localhost:3000/api/v1/";
  ...
  missingCredentials() {
    ...
    console.log(
      "- Start from the the Google Drive API node.js quickstart sample: https://developers.google.com/drive/api/v3/quickstart/nodejs#prerequisites"
    );
    console.log(
      `- Follow the steps there in order to 1) create a project, 2) enable the GDrive API, and 3) authorize credentials for a service account`
    );
    console.log(
      "- After you download the credentials JSON file from your Cloud Platform dashboard, place it in secret/credentials.json and run this script again"
    );
    console.log(
      "- see ./credentials-sample.json for an example of the format of this file (but the credentials in that sample will not work!)"
    );
    process.exit(1);
  }
  ...
  downloadImage(url) {
    ...
    request.head(url, (err, res, body) => {
      request(url)
        .pipe(fs.createWriteStream(cache_filename))
        .on("close", () => {
